...
parser.add_argument('-d', '--dataset', type=str, default='prid', choices=data_manager.get_names())
...
parser.add_argument('--use-OSMCAA', action='store_true', default=False, help="Use OSM CAA loss in addition to triplet")
parser.add_argument('--cl-centers', action='store_true', default=False, help="Use cl centers verison of OSM CAA loss")
# Architecture
parser.add_argument('-a', '--arch', type=str, default='resnet50tp', help="resnet50tp, resnet50ta")
...
def train(model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu , optimizer_center , criterion_center_loss , criterion_osm_caa=None):
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        print('Cross Entropy Loss: ', ide_loss)
        ...
        if args.use_OSMCAA:
            if use_gpu:
                ...
                print('Online Soft Mining and Center Aware Attention Loss: ', osm_caa_loss)
            else:
            ...
            print('------------------------------------------------------------------------------------------------------------------------------------------------------------------')
            print('Loss = Cross Entropy Loss + {(1 - Beta Ratio) * Triplet Loss} + (Center Loss * Center Loss weight) + (Beta Ratio * Online Soft Mining Center Aware Attention Loss)')
            print('------------------------------------------------------------------------------------------------------------------------------------------------------------------')
            print('Loss: ', loss)
...
def test_rerank(model, queryloader, galleryloader, pool, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    print()
    # print("Batch Number: ", )
    if 'prid' in args.dataset:
        print("mAP: {:.1%} vs {:.1%}".format(mAP, re_rank_mAP))
    print("CMC curve")
    for r in ranks:
        print("Rank-{:<3}: {:.1%} vs {:.1%}".format(r, cmc[r-1], re_rank_cmc[r-1]))
    print("------------------")
    if 'custom' not in args.dataset:
        print("Dataset not CUSTOM : instead", args.dataset)
        return cmc[0]
    else:
        if args.focus == "map":
            print("returning map")
...
if not args.validation_training:
    ...
    #torch.save(model.module.state_dict(), "model_.pth")
    ...
    #torch.save(model.module.state_dict(), "model__.pth")
